...
- The outer read-only PowerShell identity execution (receipt 2701) exited `0` after printing the frozen blob matches and head, while its inner unquoted `git rev-parse HEAD^{tree}` emitted a fatal ambiguous-revision diagnostic; that Git subcommand's numeric exit was not separately recorded. The quoted correction execution (receipt 2708) exited `0` and returned tree `fdb46a97219a096e8686c17f4628c15c374b61f3`. No repository state changed, and the failure is retained rather than erased. Recovered operational evidence: [complete audit](https://github.com/milocaetano/quantick/issues/491#issuecomment-5672775422) and [artifact manifest](https://github.com/milocaetano/quantick/issues/491#issuecomment-5672793062).
