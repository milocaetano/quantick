...
| `QUANTICK_FAKE_LATENCY_SPLIT=<arrival>,<source>,<transport>,<hop>` | a **late tape that knows who is late**: the status bar's tape cell carries the hop's name beside the figure (`arrival 18112 ms · bridge`) and the hover carries the numbers behind it. Milliseconds and one word, e.g. `18112,17980,132,bridge`. The name only shows above `HIGH_LAG_MS` (5 s), so a value under that photographs the healthy cell instead. The state it draws exists only while a real venue is running badly — no setting reaches it and no recording has it, since a replay has no chain to attribute — so without this hook the cell, its hop name and its whole hover breakdown are invisible to anything but a human waiting for a bad day. A malformed value is logged (`FAKE_LATENCY_SPLIT_REJECTED`) and ignored, never silently defaulted: a typo must not photograph the wrong state and call it a pass |
