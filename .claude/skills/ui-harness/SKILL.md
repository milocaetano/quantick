...
| `QUANTICK_CANDLE_WIDTH=<px>` | the zoom, scripted: **pixels per bar**, clamped to the gesture's own 0.2–160 bounds. Below ~2.7 px the chart groups bars into one candle per slot (`Viewport::bars_per_slot`), so this is also the hook that reaches the grouped state — `0.2` is as far out as the chart goes, ~20 bars per candle, with the `×N bars per candle` note bottom-left. Footprint LOD by *slot* width: ≥63 Detailed, 33–63 Compact, 10–33 Profile, 6–10 Marks, <6 Off — all four scaled by the panel's `detail_scale`, and all four withdrawn while bars are grouped |
