...
| `QUANTICK_CANDLE_WIDTH=<px>` | the zoom, scripted: **pixels per bar**, clamped to the gesture's own 1–160 bounds. One bar is one candle at every zoom (the trust law — nothing ever groups), so `1` is the deepest squeeze: a 1600 px window holds 1600 bars. Footprint LOD by candle width: ≥68 Detailed, 33–68 Compact, 10–33 Profile, 6–10 Marks, <6 Off (the Detailed floor moved 63 → 68 when the 3 px the ladder anchors its columns at entered the arithmetic it had always been missing from; `cluster` declares its own, ≥102, for the third quantity it writes) — all four scaled by the panel's `detail_scale` |
...
| `QUANTICK_FOOTPRINT_STYLE=<id>` | which of the four readings the ladder draws: `split` (default), `bidask`, `ladder`, `cluster`. Resolved through `FootprintStyle::ALL` — the same registry the panel selector and the TOML read — so a style that exists is reachable here by name; an unknown id is **logged and ignored**, never silently swapped for the default, because a typo in a validation script must not photograph the wrong style and call it a pass. `cluster` needs ~96 px of candle to show its numbers (three quantities per row against the ladder’s two): pair it with `QUANTICK_CANDLE_WIDTH=110` or it hands over to `bidask` and the legend says `cluster → bidask` |
