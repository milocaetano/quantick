...
| `QUANTICK_STRATEGY_DEMO=retired-badge` | an instance **retired** because its region can no longer cover a bar to come — the badge reading `region ended`. On a live chart the sweep reaches it only after the tape walks past a hand-drawn right edge, which is minutes of market a capture cannot wait out, so the scene stages the disarm itself. Same framing caveat as `alarm-badge`: the badge paints at the region's top-left corner, off-canvas at the default zoom |
