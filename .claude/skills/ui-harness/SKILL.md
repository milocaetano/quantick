...
| `QUANTICK_SOURCE_PICKER=1` | the **Open market dialog** on screen at launch — the feed list, the symbol list, the *Add symbol…* field and the rows a hand-added symbol can be removed from. Behind the tab strip's `+` (or Ctrl+T), which a scripted run cannot press. Goes through `SourcePickerSurface::open` on the running catalog, the same call the `+` makes, so the dialog opens on a market the config really offers rather than on a fixture |
| `QUANTICK_INDICATOR_PREVIEW=1` | the **preview watermark** over the focused chart — the line saying the numbers under it are an unapplied draft. The real one appears only while a settings dialog is previewing, which needs a dialog open, a value nudged and the worker's answer back; this stages the banner alone so it can be read for legibility against a busy canvas. It is the **only** way to reach the banner from a launch: `QUANTICK_INDICATOR_SETTINGS` opens the dialog but never previews anything — `previewed` is set by nudging a value and waiting for the worker, which no boot hook does — so the two do not pair, and a run that sets both photographs the forced banner over the focused pane. A real preview still wins where one exists, which is what keeps the hook from ever moving the banner off the pane the application chose |
...
| `QUANTICK_TOAST=paper` | a **simulator acknowledgement** in that same lane, posted through `PaperTrading::show_toast` — the panel's own door. The `plain` and `undo` values raise a message *in* the lane; this one proves the route to it: the panel's outbox, the drain every tab goes through, and the eight-second clock the surface owns. It is the only way to photograph a paper acknowledgement at all — the paper demo trades inside the first second of a launch and the message is gone eight seconds later, so a run without this hook shows an empty lane and cannot tell that from a defect |
...
| `QUANTICK_MENU=workspace` | the Workspace menu open on the first drawn frame — the only door to Save, Save as, Export, Open from file, Open recent and Show where it's saved. A menu is a popup egui owns, so there is no state to set that would not be a second way of opening it: the hook delivers the click on the button's own rect, through the app's input path (`raw_input_hook`), exactly as `QUANTICK_CONTEXT_MENU` does. See `QUANTICK_MENU=history` below for the toolbar's history group; a token this build does not know opens nothing rather than the wrong menu. |
...
| `QUANTICK_STRATEGY_DEMO=alarm-sounds` | the same dialog with the **sound picker dropped open** — the three headings (*system*, *standard alarms*, *nature alarms*) and every sound under them, the current one lit. A combo's list exists only while it is open and opening it is a click, so this is the only way a capture sees the grouping at all; the scene opens the popup through egui's own `Memory::open_popup` on the frame the dialog appears, on the id `ComboBox` derives for it (`surfaces/strategy_popup.rs`, `draw_alarm_controls`) — if a capture shows the picker closed, that derivation is the first suspect, and the first click elsewhere closes it as it would for a trader |
...
| `QUANTICK_PAPER_STRATEGY_EDITOR=1` | the **exit-strategy editor** open on the first frame: the list of named ladders on the left, the open one's rungs (share %, gain, loss in ticks) on the right, and the line that says why an unfinished ladder cannot be used. It is a window, drawn from the app's own frame rather than from the Trading tab that opens it, so this hook reaches it whatever the dock is showing and whether it is showing at all — which is also why it is a window in the first place. Reaching it by hand needs a click on `Edit…` in the ticket, which a scripted run has no hand for (the ParkedHand rule). Pair with `QUANTICK_PAPER_STATE` at a scratch path: the editor saves on close, and a capture must not rewrite the trader's own ladders. |
| `QUANTICK_PAPER_RISK=<amount>[:<point value>:<size step>:<currency>][:unlocked]` | the **risk per trade** standing up on the first frame, so the derived `Qty`, the discreet line under it and the refusal the lock produces are all reachable without a hand. `100` alone photographs the *undeclared* instrument - the state a trader sees before filling the two fields in - while `100:0.20:1:BRL` declares WIN$N's money so a size is on screen. `2%@10000:0.20:1:BRL` takes the percentage path instead. A trailing `:unlocked` takes the lock down, which is the only way a capture reaches "over budget and still placeable". Pair with `QUANTICK_PAPER_RULER_TICKS` (the stop the size is measured from) and `QUANTICK_CMD_PREVIEW` (the aim it measures from); the clamped state wants a coarse `Step` in the ticket, because the ruler itself stops at 200 notches. **`QUANTICK_PAPER_STATE` at a scratch path is mandatory**: the money and the risk are sidecar writes, and an unhooked run rewrites the trader's own. A spec that does not parse is logged `PAPER_RISK_HOOK_REJECTED` and ignored, never defaulted. |
...
Pair with `QUANTICK_LAYOUT=time+flow` and `QUANTICK_VENUE_HISTORY_DEMO=1` for the split the controls were designed against: a time pane full of the venue's candles beside a flow pane with nothing in it. The corner speaks for the window, so it appears once whatever the layout; the muted line lands on the empty pane alone.

Once merged, move it into the table above.

Landing with the offline-corner goal (`feat/offline-corner-last-session`):
