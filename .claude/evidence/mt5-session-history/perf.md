...
First bars on the chart: **3.45 s** after `MT5_BACKFILL_START`, **6.70 s** after process launch.
...
`APP_SLOW_FRAMES` inside the fill: **0**
First bars on the chart: **1.31 s** after `MT5_BACKFILL_START`, **5.92 s** after process launch.
...
on the session. From *process* launch both are around six seconds, nearly all
of it the window opening and the terminal answering, which is the same before
and after and is not this branch's to spend.
