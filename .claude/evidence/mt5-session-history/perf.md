...
First bars on the chart: **3.45 s** after `MT5_BACKFILL_START`, **6.70 s** after this log's first line.
...
`APP_SLOW_FRAMES` inside the fill: **0**
First bars on the chart: **1.31 s** after `MT5_BACKFILL_START`, **5.92 s** after this log's first line.
...
| first bars on the chart | 3.45 s | **1.31 s** | −2.14 s |
...
arrives, and the fill runs for the ten seconds after that.

**The branch puts bars on the screen sooner than the control does**, with eight
times the data behind them — 1.31 s against 3.45 s from `MT5_BACKFILL_START`.
That is the point of the opening block: the trader waits on 200 000 prints, not
on the session. From each log's own first line both are around six seconds, nearly all of it
the window opening and the terminal answering — the same before and after, and
not this branch's to spend. That anchor is deliberately not called "process
launch": it is the earliest instant a log file can see, which is already after
the process started, and this is not the file to round an approximation up in.

(Those two timings are generated by `summarise_perf.py` as of this round. An
earlier revision of this paragraph said the block was on screen "within a
second of launch", then corrected that to 5.92 s while naming the abandoned
50 000-slice run — a third log again, whose own timings are neither of these.
The number was right and the sentence pointed somewhere else, which is the
defect this file exists to stop, so the timing is read off the log now instead
of being computed by hand.)
