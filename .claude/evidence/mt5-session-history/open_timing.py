...
def report(marks, ticks, first_ms, last_ms, bytes_in) -> int:
    ...
    print(f"opening block     : {ticks} ticks, {bytes_in / 1e6:.0f} MB")
    print(f"  from            : {stamp(first_ms)}")
    print(f"  to              : {stamp(last_ms)}")
    print(f"  span            : {(last_ms - first_ms) / 3_600_000:.2f} h")
    print()
    for name in ("connected", "hello", "backfill_start", "first tick", "backfill_end"):
        if name in marks:
