...
| --- | --- | --- |
| Normal 1400x900 split over live BTC data | PASS | `wide-distinct-layouts.png`: both panes have separate footers; the left pane marks `Levels` and the right marks `Layout 1`; divider, axes, status bar and charts do not overlap. The captured frame reports 60 fps and the launch health reports zero worker backlog. |
