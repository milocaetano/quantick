...
- **R4** — Deliver it as a draft PR against `campaign/lean-a-plus`, reviewed
  ...
  touch main.
- **R5** — (Coordinator, mid-task) Merge the campaign's new tip (`f74c0cbd`,
  #436) into this branch as a normal merge commit, no rebase of the main
  merge, resolving any overlap with #306 and rerunning the checks, so that
  the PR's base diff is main's delta only.
...
- **D8** — (Coordinator, mid-task) The new campaign tip is merged into the
  branch with a merge commit; everything else in the brief stands.
...
- ~~**S3** — The UI behaviour matrix needs no new rows: its generator and
  drift guard pass unchanged after the merge, and #306/#420 registered their
  own operability entries on main.~~ Withdrawn after delivery-review: the
  premise was false — main has no UI behaviour matrix (it is campaign-only),
  so no row came from main, and the drift guard cannot see #420's raw
  Ctrl+C/Ctrl+V keys. The rows D4 asks for are written in this branch and
  graded under A7.
...
- [ ] **A7** — Generated artifacts are regenerated by their commands; the
      ...
      `layout.pane.set_bar_spec`; the UI behaviour matrix has a row for #306's
      REC control (`feed.deal_recording.set`), maps `chart.bars.set_spec` to
      `layout.pane.set_bar_spec`, and has a row for #420's copy and paste
      (`drawing.duplicate`, a recorded `pending_capability` exclusion — #420
      adds no control call, so it takes no retry row); the released-schema
      compatibility tests pass with no released baseline regenerated.
      *Evidence:* dump commands, `retry_matrix` and `operability` tests, the
      ...
      schema compatibility tests, and `git diff` showing
      `schemas/control/released/` untouched. → PR body. *(R3)*
- [ ] **A8** — The campaign's fixed version of #361's test (#382) is kept.
      *Evidence:* `git diff 6ba1a7f3 -- crates/app/src/worker_progress/` is
      empty. → PR body. *(R2)*
- [ ] **A9** — A draft PR with base `campaign/lean-a-plus` exists and is
      never merged by this mission. *Evidence:* `gh pr view` state.
      → handoff. *(R4)* (Readiness moved to closing step C4: it waits on this
      review, so it cannot be a criterion this review grades.)
- [ ] **A10** — Q3's live envelope (#405) holds with #306 composed in: the
      envelope counters read the tape's length, #306's readings are not
      counted as prints, and the envelope tests pass.
      *Evidence:* `health/envelope.rs` unchanged by the merge;
      `app/health.rs` +1 line (the deal status cell);
      `inside_the_envelope_every_print_arrives_and_no_queue_fills`,
      `above_the_envelope_a_stalled_worker_parks_and_folds_and_loses_nothing`,
      `a_pane_past_the_envelope_warns_once_per_crossing` pass. → PR body.
      *(R2)*
- [ ] **A11** — The merge touches the sibling Q13's files only as main
      requires: nothing under `crates/app/src/control/gateway/` differs from
      the campaign tip. *Evidence:* `git diff origin/campaign/lean-a-plus HEAD
      -- crates/app/src/control/gateway/` empty. → PR body. *(R2)*
- [ ] **A12** — The campaign's tip `f74c0cbd` (#436) is merged into the
      branch by a merge commit, the main merge `260ee35e` is not rewritten,
      and the PR's diff against its base is main's delta plus this mission's
      own repairs. *Evidence:* `git log -1 --format=%P` of the merge commit,
      `git merge-base --is-ancestor origin/campaign/lean-a-plus HEAD`, the
      four checks after it. → PR body. *(R5)*
...
- [ ] **G5** — Adds something a trader does (#306's REC and bar rule, #420's
      ...
      inventory, retry-matrix and UI-matrix rows for the two new capabilities;
      #420's copy/paste is keyboard-first and its missing capability is
      recorded in the UI matrix. → PR body.
- [ ] **G6** — The brief's extra validation (D5): `cargo test -p
      quantick-guards`, `sh .claude/hooks/guardrails_test.sh`, `ruff check
      --select F bridge/mt5 tools/mt5`, `python bridge/mt5/tests/test_*.py`,
      `python tools/mt5/test_export_session.py`. *Evidence:* logs and the PR
      body's verification section. → PR body.
...
- **C4** — `gh pr ready` requested once, via Bash with the cd prefix, after
...
> Coordinator FYI for X2: PR #436 (Q13) just merged into `campaign/lean-a-plus`. It changes `crates/app/src/control/gateway/server.rs` (new `server/answer.rs`, `InFlightId`, `answer_and_release`), the control-plane test modules, `crates/app/src/app/tests/mod.rs` (`pause_one_request_interval` moved there), `retry_readback_tests.rs`, the live-envelope burst test and `docs/quality/live-envelope.md`. Before opening your PR (or now, if you are past the main merge), fetch and merge the new campaign tip into `sync/main-d8de9a1b` as a normal merge commit (no rebase of your merge), resolve any overlap with main's #306 changes in the same files, and rerun the checks. Your PR's base diff must be the main delta only. Everything else in your brief stands.
