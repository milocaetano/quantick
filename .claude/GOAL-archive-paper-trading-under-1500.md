...
> You are executing campaign child mission **S2** of campaign #367 for the
> repository milocaetano/quantick: split `crates/app/src/paper_trading.rs`
> (4,589 production lines), `crates/app/src/paper_report.rs` (3,300) and
> `crates/app/src/paper_account.rs` (2,128) into owned sibling modules, each
> at most 1,500 production lines, with no behaviour change on the money path.
> You are a subagent: you cannot ask the trader anything; the coordinator has
