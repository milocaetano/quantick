...
- [ ] **A14** Original issue source and received delegation/host directives are preserved as attributed verbatim quotations; original IDs and issue D3 text remain intact with D4 supersession explicit. Evidence: this archive. (R16)
...
- **A11 / R13 ? historical report-before-repair receipts only.** Historical grade remains UNPROVEN for absent original private report bodies. Actual retained adverse/final samples, unchanged thresholds, binary identities and cumulative failure counters remain mandatory and are not deferred.
- **A14 / R16 ? original private directive-body fidelity only.** Historical grade remains UNPROVEN where bodies are unavailable. Retained issue text, attributed archive quotations, stable IDs and explicit D3/D4 supersession remain required.
- **A22 / R25 ? original pre-host submission content only.** Historical grade remains UNPROVEN. Actual send/release/use timestamps remain evidence only of their observable order, not hidden message contents.
