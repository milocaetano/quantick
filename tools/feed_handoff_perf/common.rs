...
async fn drain(rx: &mut mpsc::Receiver<FeedEvent>, totals: &mut Totals, target: u64) {
    while totals.trades < target {
        totals.observe(rx.recv().await.expect("host ended before qualifying trade"));
    }
}

fn channels() -> (mpsc::Sender<FeedEvent>, mpsc::Receiver<FeedEvent>, mpsc::Sender<FeedNotice>, mpsc::Receiver<FeedNotice>, mpsc::Sender<FeedCommand>, mpsc::Receiver<FeedCommand>) {
...
async fn binance(fixture: &serde_json::Value) -> (u128, Totals) {
    ...
    let (tx,mut rx,notice_tx,_notice_rx,cmd_tx,cmd_rx) = channels();
...
async fn hyperliquid(fixture:&serde_json::Value,case:&str)->(u128,Totals) {
    ...
    let listener=TcpListener::bind("127.0.0.1:0").await.unwrap();
    let url=format!("ws://{}",listener.local_addr().unwrap());
    let (tx,mut rx,notice_tx,mut notice_rx,cmd_tx,cmd_rx)=channels();
    ...
    for (index,payload) in payloads.into_iter().enumerate() {
        ...
        let prior_anomalies=totals.anomalies;
        ...
        assert_eq!(totals.anomalies-prior_anomalies,if diagnostics {2}else{0});
    ...
    assert_eq!(totals.anomalies,expected*2); assert_eq!(totals.missing,expected*2);
    assert_eq!(totals.non_monotonic,expected); assert_eq!(totals.unknown,0);
