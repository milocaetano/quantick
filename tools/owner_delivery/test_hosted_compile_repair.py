...
class CompileRepairs(unittest.TestCase):
    ...
    def run_case(self, record, accepted):
        with tempfile.TemporaryDirectory() as folder:
            root=Path(folder).resolve(); (root/'receipts').mkdir()
    ...
    def disposal(self, mutation=None, reason=None):
        with tempfile.TemporaryDirectory() as folder:
            root=Path(folder).resolve(); target=root/'B-target';target.mkdir()
            ...
            with patch.object(hosted,'ROOT',root):
                if mutation:
                    with self.assertRaisesRegex(owner.InvalidEvidence,reason): hosted.dispose_intermediates(target,keep,root/'disposal.json',closures)
    ...
    def test_failed_profile_prevents_any_deletion(self): self.disposal(lambda c,k:c[0].update(state='failed'),'both side profiles must close')
    def test_unresolved_tree_prevents_any_deletion(self): self.disposal(lambda c,k:c[0]['supervision'].update(tree_quiescent=False),'owned compile tree is not quiescent')
    def test_artifact_mismatch_prevents_any_deletion(self): self.disposal(lambda c,k:k.update({next(iter(k)):'bad'}),'retained artifact changed')
    def test_junction_member_refused(self):
        with tempfile.TemporaryDirectory() as folder:
            root=Path(folder).resolve();target=root/'C-target';target.mkdir(); item=target/'item';item.write_bytes(b'kept')
            with patch.object(hosted,'ROOT',root),patch.object(Path,'is_junction',lambda p:p==item):
                with self.assertRaisesRegex(owner.InvalidEvidence,"unsafe target member"): hosted.dispose_intermediates(target,{},root/'receipt.json',[dict(state='terminal',supervision=success())]*2)
            self.assertTrue(item.exists())
    def test_other_target_refused(self):
        with tempfile.TemporaryDirectory() as folder:
            root=Path(folder).resolve()
            with patch.object(hosted,'ROOT',root):
                with self.assertRaisesRegex(owner.InvalidEvidence,'not a fresh owned side target'): hosted.dispose_intermediates(root/'other',{},root/'receipt.json',[])
    ...
    def test_noncanonical_root_refused_without_resolve_mock(self):
        with tempfile.TemporaryDirectory() as folder:
            canonical=Path(folder).resolve(); (canonical/'child').mkdir(); (canonical/'B-target').mkdir()
            root=canonical/'child'/'..'
            self.assertNotEqual(root.absolute(),root.resolve())
            with patch.object(hosted,'ROOT',root):
                with self.assertRaisesRegex(owner.InvalidEvidence,'target root redirected'):
                    hosted.dispose_intermediates(root/'B-target',{},canonical/'receipt.json',[])
