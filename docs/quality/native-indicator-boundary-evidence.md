...
Protected implementation items accept canonical root names or qualified paths ending in them, with optional leading `::`, for inherent implementations or non-generic path-named trait implementations. Whitespace and line breaks in these headers are immaterial. Generics, where clauses, references/associated self types, unsafe/default modifiers, attributes and other protected headers outside that subset receive an unsupported-scope diagnostic. `impl Trait` inside parameter/type delimiters or after a return arrow is not an implementation item. Ordinary implementation braces are scanned, including nested explicit items, and the per-root union avoids double counting.
