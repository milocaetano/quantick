...
| A5 | 10 | **Harness out of production.** `ui_crate.harness_hooks`, and where capture, demo and automation hooks run. | 5: every hook compiles only under a cargo feature or `cfg(test)`. 4: ≤20 names, read in one launch module, none checked on a per-trade or per-frame path. 3: hooks live in dedicated modules and number ≤150. 2: hooks spread into domain-adjacent code, or >150. 1: a hook alters feed, order or persisted data in a release build without an explicit opt-in. |
