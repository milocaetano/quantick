...
This is the PR 0 migration baseline, measured at `fcd2ac4` and superseded
by the generated [capability inventory](../capability-inventory.md). It is
kept because it records the `QUANTICK_*` surfaces the control plane was
designed to replace and the migration target chosen for each, which the
generated file does not carry and should not: that is a decision record,
not a description of the registry.
