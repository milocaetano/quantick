# Control plane roadmap, sections 1-8 (August 2026)
...
These are sections 1 to 8 of `roadmap.md` as they stood on 2026-08-26,
when the observer stack was still open. They are moved here whole rather
than edited: the merge order, the gaps each pull request carried forward,
the remaining-work breakdown, the gates section 5 was written against, the
MVP closure map and the 2026-08-24 review findings are all correct as a
record of that moment, and all wrong as a statement of today.

Section 1's delivery table came here with the rest. It was the document's
most-read half and its least true: it recorded PR 5c as open and PR 6 as not
started long after both had shipped, which is precisely the claim a reader
acts on. What replaced it in [`../roadmap.md`](../roadmap.md) states no
delivery status at all, because the registry already answers that question
and a second answer can only disagree with the first.

The brief that ordered this move named sections 2 to 5. Sections 1 and
6 to 8 came with them because they cannot stand without section 5: section 6 is
titled "Gates for every pull request in section 5", and sections 7 and 8
state the delivery status of a stack that has since closed — exactly the
claim no document outside this directory is allowed to make.
...
the stack's branches, `history/pr2-performance.md`, `history/pr3-gateway-evidence.md`,
`history/pr4-mcp-evidence.md` and `history/pr5a-events-evidence.md` in this directory.
