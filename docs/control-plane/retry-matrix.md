...
| `annotate.remove` | 1 | forbidden | key refused; not retryable, read back | `annotator` | `snapshot.read` `analysis.drawings` | `tabs[].panes[].drawings[].drawing_id` | the named `annotation_id` is no longer listed | `an_interrupted_annotation_is_resolved_by_its_readback`, `every_reachable_forbidden_row_refuses_a_key_before_the_application` |
...
| `layout.focus.set` | 1, 2 | optional | deduplicated per connection | `cockpit` | `snapshot.read` `workspace.summary` | `tabs[].panes[].focused` | the pane at the address asked for is the focused one (per pane: two context charts share a side) | `every_reachable_optional_row_replays_a_dropped_answer_and_begins_once`, `layout_v2_answers_with_the_exact_share_and_v1_is_still_there` |
...
| `layout.pane.set_interval` | 1, 2 | optional | deduplicated per connection | `cockpit` | `snapshot.read` `chart.summary` | `panes[].bar_spec` | the pane's bar spec is the interval asked for | `every_reachable_optional_row_replays_a_dropped_answer_and_begins_once`, `layout_v2_answers_with_the_exact_share_and_v1_is_still_there` |
...
| `layout.tab.rename` | 1 | optional | deduplicated per connection | `cockpit` | `snapshot.read` `workspace.summary` | `layouts[].name` | the layout carries the name asked for | `every_reachable_optional_row_replays_a_dropped_answer_and_begins_once`, `layout_v2_answers_with_the_exact_share_and_v1_is_still_there` |
| `layout.tab.switch` | 1 | optional | deduplicated per connection | `cockpit` | `snapshot.read` `workspace.summary` | `layouts[].active` | the layout asked for is the active one | `every_reachable_optional_row_replays_a_dropped_answer_and_begins_once`, `layout_v2_answers_with_the_exact_share_and_v1_is_still_there` |
