...
| `layout.tab.switch` | 1 | optional | deduplicated per connection | `cockpit` | `snapshot.read` `workspace.summary` | `layouts[].active` | the layout asked for is the active one | `every_reachable_optional_row_replays_a_dropped_answer_and_begins_once` |
