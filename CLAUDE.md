...
- **English is the repository's language**: identifiers, comments, doc comments, log/error/UI strings, test names, `.pine` scripts, config comments, docs, commit messages, branch names and PR titles/bodies are all written in English. Sessions with the trader happen in whatever language they speak — the rule starts where something lands in a tracked file. The sole exemption is content whose *subject* is a language: localisation resources, a fixture reproducing a system's own localised string (translating it would make the fixture lie), or a marked quotation; the code, comment and test name around it stay English. `arch-review` dimension 8 grades this and a violation blocks the PR.
