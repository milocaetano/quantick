...
- `strategy` (package `quantick-strategy`) — deterministic strategy kernel, the brain the chart's live paper trading and the backtest both arm: closed bars plus a human-drawn price region in, `sim` commands out. Owns the trigger port (`Trigger`, force-bar ruler first), the region test, projected protective brackets and the armed-instance state machine (armed → fired → in position → done/disarmed, every disarm named). A pure domain crate like `engine`: no UI, no network, no async, no wall clock; depends only on `engine` and `sim`.
