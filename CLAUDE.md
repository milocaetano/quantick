...
- `mcp` (package `quantick-mcp`) — the MCP adapter, first of the control plane's adapters: a local STDIO server that discovers a Quantick instance *already running* with local agent access enabled, authenticates through `control-local`, and exposes a small fixed tool set (`quantick_describe`, `quantick_get_snapshot`, `quantick_get_chart_window`, `quantick_get_diagnostics`, `quantick_search_capabilities`, `quantick_invoke`) over the gateway's observer capabilities. A leaf: depends on `control` and `control-local` only, never on `app` or a domain crate; it never starts the application, and its stdout carries MCP frames only.
