...
declare_stages! {
    pub enum FrameStage {
        ...
        HistoryNoteHook after [ExpireHistoryNotes],
        ...
        EvidenceHook after [AnnotateHooks],
        /// The control gateway services this frame's requests. After the
        /// evidence hook, which must capture before the gateway drains.
        GatewayService after [EvidenceHook],
        /// Scripted views, demos and pending history requests.
        ScenarioHooks after [],
        /// Window startup, the health summary and workspace upkeep. After
        /// the drain: the summary reports the trades it counted.
