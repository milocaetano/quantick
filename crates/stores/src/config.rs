...
/// The shipped default, embedded so a fresh checkout opens on a chart. The
...
pub struct FeedConfig {
    ...
    /// (see `quantick_chart::state::BarSpec::parse`). It sets the flow pane's opening
...
impl AppConfig {
    ...
    #[must_use]
    pub fn startup_spec_for(&self, id: &str) -> Option<quantick_chart::state::BarConfiguration> {
...
/// Load the config from `explicit` when the caller resolved one, else from
/// `quantick.toml` in the working directory, else the embedded default.
///
...
pub fn load_from(
    ...
    let config =
        parse_with_additions(EMBEDDED_DEFAULT, ConfigSource::Embedded, &added, added_path)?;
...
mod tests {
    ...
    fn the_shipped_default_opens_two_charts_with_the_flow_pane_on_tick_bars() {
        ...
        assert_eq!(
            ...
            Some(quantick_chart::state::BarSpec::Tick(50).into()),
    ...
    fn every_declared_bubble_preset_resolves_in_the_shipped_presets_file() {
        let presets = crate::bubble_presets::parse(include_str!("../../app/config/bubbles.toml"))
    ...
    fn declared_layout_and_bars_are_read_and_resolved() {
        ...
        assert_eq!(
            ...
            Some(quantick_chart::state::BarSpec::Time(300_000).into())
        );
        ...
        for key in [r#"default_bars = "trades:2000""#, "record_deals = true"] {
            ...
            if key.starts_with("default_bars") {
                assert_eq!(
                    ...
                    Some(quantick_chart::state::BarSpec::Trades(2000).into())
                );
            }
        }
    }
