...
pub const UI_STATE_FILE: &str = "ui-state.toml";
...
///
/// `clamp_split` is the divider's own clamp, handed in by the window that
/// owns the divider: a hand-edited fraction must land exactly where a drag
/// could have left it, and two constants for one domain is how they drift.
...
pub fn restore(
    ...
    config: &AppConfig,
    clamp_split: fn(f32) -> f32,
) -> Workspace {
    workspace.active_tab = filter_tabs(
        &mut workspace.tabs,
...
#[cfg(test)]
fn named<'a>(workspace: &'a Workspace, name: &str) -> Option<&'a NamedArrangement> {
    workspace.saved.iter().find(|entry| entry.name == name)
}
...
pub fn filter_tabs(
    ...
    for tab in tabs.iter_mut() {
        ...
        tab.split_fraction = tab.split_fraction.map(clamp_split);
...
mod tests {
    ...
    /// The divider's range a test restores into: any clamp will do, since
    /// the rule under test is that the file's value goes through it.
    fn clamp(fraction: f32) -> f32 {
        fraction.clamp(0.2, 0.8)
    }
    ...
    fn a_hand_edited_divider_is_brought_back_inside_the_range_the_drag_enforces() {
        ...
        assert_eq!(restored.tabs[0].split_fraction, Some(clamp(0.99)));
        assert_eq!(restored.tabs[1].split_fraction, Some(clamp(-3.0)));
    }
    ...
    fn the_active_tab_never_points_past_what_survived() {
        ...
        assert_eq!(restore(workspace, &catalogue(), clamp).active_tab, 0);
    }
    ...
    fn a_bookmark_goes_through_the_same_gate_as_the_startup_screen() {
        ...
        let restored = restore(workspace, &catalogue(), clamp);
        let entry = named(&restored, "mixed").expect("the bookmark survives");
    ...
    fn a_bookmark_with_nothing_left_to_open_is_forgotten() {
        ...
        let restored = restore(workspace, &catalogue(), clamp);
        assert!(
            named(&restored, "all-gone").is_none(),
