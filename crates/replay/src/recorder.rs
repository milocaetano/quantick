...
use quantick_civil::{CivilDate, TzOffset, civil_utc};
...
type Recording = crate::deals::DealRecordingWriter;

/// Read one recording through the headless replay-owned codec.
pub fn read_file(path: &Path) -> io::Result<crate::deals::DealFile> {
    crate::deals::read_file(path)
}

/// Cache entries use the same headless day metadata every consumer sees.
pub type DayCache = crate::deals::DayCache;

/// Every recorded day under \`dir/symbol\`, oldest first.
#[must_use]
pub fn scan_days(dir: &Path, symbol: &str, cache: &mut DayCache) -> Rc<[RecordedDay]> {
    let folder = dir.join(crate::deals::sanitize_symbol(symbol));
    Rc::from(
        crate::deals::scan_days(&folder, cache)
            .into_iter()
            .map(|day| RecordedDay {
                day: day.day,
                first: day.first,
...
impl DealRecorder {
    ...
    fn open_day(&mut self, day: &str) -> Vec<DealSample> {
        self.close_recording();
        let folder = self.dir.join(crate::deals::sanitize_symbol(&self.symbol));
        ...
        if let Some(mut recording) = self.recording.take() {
            ...
            if let (Some(first), Some(last)) = (recording.first, recording.last) {
                let stamp = fs::metadata(&recording.path)
                    ...
                    .unwrap_or((0, None));
                let day = crate::deals::RecordedDealDay {
...
pub fn day_of(time_ms: i64, tz_minutes: i32) -> String {
    ...
    CivilDate::from_ms(time_ms, TzOffset::new(tz_minutes)).iso()
}

/// `HH:MM:SS` of `time_ms` in the display timezone.
#[must_use]
pub fn fmt_hms(time_ms: i64, tz_minutes: i32) -> String {
    // The civil clock the time axis reads too: one arithmetic, one spelling.
    let (_, _, _, hours, minutes, seconds) =
        civil_utc(time_ms.saturating_add(TzOffset::new(tz_minutes).offset_ms()));
...
pub fn fmt_count(n: u64) -> String {
    ...
    crate::deals::thousands(usize::try_from(n).unwrap_or(usize::MAX))
}
...
mod tests {
    ...
    use crate::deals::HEADER;
    use crate::scratch::Scratch as ScratchDir;
