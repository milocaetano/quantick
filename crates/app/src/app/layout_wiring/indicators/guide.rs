...
pub(super) fn queue_saved(app: &mut QuantickApp, owner: TabSlot, entry: &SavedIndicator) {
    if entry.mouse_vertical_line {
        app.indicators.pending_mouse_vertical_lines.push(owner);
    }
}

pub(super) fn apply_pending(app: &mut QuantickApp) {
...
fn set_at(app: &mut QuantickApp, origin: TabSlot, enabled: bool) -> bool {
