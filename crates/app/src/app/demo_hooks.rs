...
impl QuantickApp {
    ...
    pub(super) fn apply_drawing_demo(&mut self) {
        if !self.surfaces.drawing_chrome.demos().gallery_requested() {
            return;
        }
        let slots = self.active_tab().flow_pane.slots();
        let Some(request) = self.surfaces.drawing_chrome.demos_mut().take_gallery(slots) else {
            return;
        };
        if request.shared {
        ...
        if self.surfaces.drawing_chrome.demos().recut_requested() {
            let pane = &mut self.active_tab_mut().flow_pane;
    ...
    pub(super) fn apply_drawing_draft(&mut self) {
        ...
        let Some(mut plan) = self.surfaces.drawing_chrome.demos_mut().take_draft(
    ...
    pub(super) fn apply_frvp_demo(&mut self) {
        let Some(request) = self.surfaces.drawing_chrome.demos().profile_requested() else {
            return;
        };
    ...
    pub(super) fn apply_avwap_demo(&mut self) {
        if !self.surfaces.drawing_chrome.demos().avwap_requested() {
            return;
        }
        let slots = self.active_tab().flow_pane.slots();
        let Some(geometry) = self.surfaces.drawing_chrome.demos_mut().take_avwap(slots) else {
            return;
        };
        let pane = &mut self.active_tab_mut().flow_pane;
