...
pub(super) fn draw_bar(
    ...
    let bar = BarRender::new(paint, rows, xc);
    bar.paint_plate();
    for (&row, cell) in rows {
        if *cells_left == 0 {
            return;
        }
        let Some(geometry) = bar.row_geometry(row, cell) else {
        ...
        *cells_left -= 1;
        bar.paint_row(cell, &geometry);
    }
    bar.paint_extreme_badges();
}

/// One bar's render context: the frame's lent facts plus what this bar alone
/// decides once — its rows, its x, its POC and the three scales its rows are
/// measured against. The paint passes are its methods, in the order the bar
/// is painted: the plate, then each row in the style in force, then the
/// extreme badges on top.
///
/// A plain value on the stack, built once per bar: nothing here allocates.
struct BarRender<'p, 'f> {
    paint: &'p BarPaint<'f>,
    frame: &'p LayerFrame<'f>,
    rows: &'p BTreeMap<i64, FootprintLevel>,
    xc: f32,
    poc: Option<i64>,
    /// The bar's busiest row by total volume — the silhouette's full width.
    max_volume: f64,
    /// The bar's largest absolute row delta — the split's full delta bar.
    max_abs_delta: f64,
    /// The bar's largest single side — the `bidask` mirror's shared scale.
    max_side_volume: f64,
}

impl<'p, 'f> BarRender<'p, 'f> {
    fn new(paint: &'p BarPaint<'f>, rows: &'p BTreeMap<i64, FootprintLevel>, xc: f32) -> Self {
    ...
    fn dominates(&self, qty: Decimal, other: Decimal) -> bool {
        qty >= self.paint.ratio.saturating_mul(other)
            && qty.saturating_sub(other) >= self.paint.min_qty
    }

    fn neighbour(&self, row: i64, side: Side) -> Decimal {
        self.rows
            .get(&row)
            .map(|l| match side {
                Side::Buy => l.buy,
    ...
    /// Where `row` lands and what the signals say about it, or `None` when it
    /// is off screen.
    fn row_geometry(&self, row: i64, cell: &FootprintLevel) -> Option<RowGeometry> {
        let frame = self.frame;
        let (top, bottom) = row_band(frame, row, self.paint.row_group);
        ...
        Some(RowGeometry {
            ...
            top,
            bottom,
            row_height: (bottom - top).max(1.0),
            is_poc: self.poc == Some(row),
            buy_imbalance: self.dominates(cell.buy, self.neighbour(row - 1, Side::Sell)),
            sell_imbalance: self.dominates(cell.sell, self.neighbour(row + 1, Side::Buy)),
        })
    }
    ...
    fn paint_plate(&self) {
        ...
        if level >= DetailLevel::Profile
            && let (Some(&first), Some(&last)) =
                (self.rows.keys().next(), self.rows.keys().next_back())
        {
            ...
            frame
                .painter
                .rect_filled(box_rect, egui::Rounding::same(2.0), plate);
            if inset > 0.0 {
    ...
    /// One on-screen row, in the style in force.
    ///
    /// Styles that own their whole row paint it themselves; the two that
    /// share the LOD ladder's generic cell fall through to the level's own
    /// row. `draws_own_rows` is what decides, so a style added to the
    /// registry declares which half it belongs to instead of being written
    /// into a condition here.
    fn paint_row(&self, cell: &FootprintLevel, geometry: &RowGeometry) {
        let &BarPaint {
            frame,
            level,
            style,
        ...
        if style.draws_own_rows() && level >= DetailLevel::Profile {
            match style {
                crate::footprint_config::FootprintStyle::BidAsk => draw_bidask_row(
                    frame,
                    cell,
                    geometry,
                    self.xc,
                    self.max_side_volume,
                    row_group,
                ),
                crate::footprint_config::FootprintStyle::Cluster => {
                    draw_cluster_row(frame, level, cell, geometry, self.xc, heat, self.max_volume);
                }
                _ => self.paint_split_row(cell, geometry),
            }
            return;
        }
        match level {
            DetailLevel::Profile => self.paint_profile_row(cell, geometry),
            DetailLevel::Compact => self.paint_compact_row(cell, geometry),
            DetailLevel::Detailed => self.paint_ladder_row(cell, geometry),
            DetailLevel::Marks | DetailLevel::Off => {}
        }
    }

    /// The reference look, inside the candle: a central axis at the candle's
    /// middle, the total-volume profile growing rightward in neutral light
    /// (the exocharts silhouette), and a delta bar per row growing leftward in
    /// the winner's color — "who won the fight" readable at a glance, the
    /// volume shape behind it.
    fn paint_split_row(&self, cell: &FootprintLevel, geometry: &RowGeometry) {
        let frame = self.frame;
        ...
        let volume_frac = (cell.volume().to_f64().unwrap_or(0.0) / self.max_volume) as f32;
        let delta = cell.delta();
        let delta_frac = (delta.to_f64().unwrap_or(0.0).abs() / self.max_abs_delta) as f32;
        let reach = (frame.half - 1.0).max(1.0);
        ...
        if self.paint.level == DetailLevel::Detailed
        ...
        if is_poc {
            ...
            draw_poc_line(frame, xc, xc + frame.half, row, self.paint.row_group);
        }
    }

    /// Profile: a textless histogram, anchored on the candle's left edge and
    /// colored by the row's delta sign — neutral by design, so the POC and
    /// the zone marks stay the only saturated things.
    fn paint_profile_row(&self, cell: &FootprintLevel, geometry: &RowGeometry) {
        let frame = self.frame;
        let xc = self.xc;
        let frac = (cell.volume().to_f64().unwrap_or(0.0) / self.max_volume) as f32;
        ...
        frame.painter.rect_filled(
            egui::Rect::from_min_max(
                egui::pos2(xc - frame.half, geometry.top + 0.5),
                egui::pos2(xc - frame.half + width, geometry.bottom - 0.5),
            ),
            egui::Rounding::ZERO,
            color.gamma_multiply(if geometry.is_poc { 0.55 } else { 0.28 }),
        );
        if geometry.is_poc {
            draw_poc_dot(frame, xc, geometry.row, self.paint.row_group);
        }
    }

    /// The number rows' font: it answers to the row height AND the cell
    /// width. A five-glyph quantity ("58.1k") is ~3 em of monospace per side,
    /// and a number that outgrows its candle bleeds into the neighbour — worse
    /// than a smaller number.
    fn number_font(&self, geometry: &RowGeometry) -> egui::FontId {
        let half = self.frame.half;
        let width_budget = match self.paint.level {
            DetailLevel::Detailed => (half - 6.0) / 3.0,
            _ => (2.0 * half - 6.0) / 3.0,
        };
    ...
    /// A ring around the POC row, not a wash under it.
    ///
    /// The old tint cost the row its contrast (6.8:1 → 4.4:1 — below AA on
    /// the one row the trader reads first) to say something an outline says
    /// louder, and the full-width line that came with it struck straight
    /// through both number columns. The split style already refuses that line
    /// for exactly this reason; the ladder had never been told. The ring also
    /// gives the POC a *shape*: the only framed row in the bar, readable
    /// without relying on hue.
    fn paint_poc_ring(&self, geometry: &RowGeometry) {
        if !geometry.is_poc {
            return;
        }
        let (xc, half) = (self.xc, self.frame.half);
        self.frame.painter.rect_stroke(
            egui::Rect::from_min_max(
                egui::pos2(xc - half + 0.5, geometry.top + 0.5),
                egui::pos2(xc + half - 0.5, geometry.bottom - 0.5),
            ),
            egui::Rounding::ZERO,
    ...
    /// Compact: the POC ring and one delta number, centred and side-inked.
    fn paint_compact_row(&self, cell: &FootprintLevel, geometry: &RowGeometry) {
        let font = self.number_font(geometry);
        self.paint_poc_ring(geometry);
        if !self.frame.config.show_numbers {
            return;
        }
        let mid = (geometry.top + geometry.bottom) / 2.0;
        let delta = cell.delta();
        ...
        self.frame.painter.text(
            egui::pos2(self.xc, mid),
    ...
    /// Detailed ladder: the POC ring, then sell | buy — the tape's own
    /// left-to-right: taker-sells hit the bid printed on the left, taker-buys
    /// lift the ask on the right.
    fn paint_ladder_row(&self, cell: &FootprintLevel, geometry: &RowGeometry) {
        let frame = self.frame;
        let painter = frame.painter;
        let xc = self.xc;
        let (top, bottom) = (geometry.top, geometry.bottom);
        let font = self.number_font(geometry);
        self.paint_poc_ring(geometry);
            ...
            if imbalanced {
                ...
                let edge = match side {
                    Side::Sell => egui::Rect::from_min_max(
                        ...
                        egui::pos2(cell_rect.left() + IMBALANCE_EDGE_PX, cell_rect.bottom()),
                    ),
                    Side::Buy => egui::Rect::from_min_max(
                        egui::pos2(cell_rect.right() - IMBALANCE_EDGE_PX, cell_rect.top()),
                        egui::pos2(cell_rect.right(), cell_rect.bottom()),
                    ),
                };
                painter.rect_filled(edge, egui::Rounding::ZERO, theme::side_color(side));
            }
    ...
    fn paint_extreme_badges(&self) {
        if self.paint.level == DetailLevel::Detailed && self.frame.config.extreme_ratio_badge {
            for extreme in [Extreme::Low, Extreme::High] {
                self.paint_extreme_badge(extreme);
            }
        }
    }

    fn paint_extreme_badge(&self, extreme: Extreme) {
        let frame = self.frame;
        let painter = frame.painter;
        let cell = match extreme {
            Extreme::Low => self.rows.iter().next(),
            Extreme::High => self.rows.iter().next_back(),
        };
        let Some((&row, cell)) = cell else { return };
        ...
        let (band_top, band_bottom) = row_band(frame, row, self.paint.row_group);
        ...
        let anchor = align.anchor_size(egui::pos2(self.xc, y), galley.size());
        painter.rect_filled(
            anchor.expand(2.0),
            egui::Rounding::same(2.0),
            canvas_backdrop(),
