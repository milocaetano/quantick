...
///
/// The window is a column of sections, each a small view that owns the knobs
/// it draws and answers whether one of them moved. A new knob docks in the
/// section it belongs to, and the section order is read here in one place.
pub fn draw(ctx: &egui::Context, input: PanelInput<'_>) -> PanelOutcome {
    ...
    egui::Window::new(format!("footprint settings — {target}"))
        ...
        .show(ctx, |ui| {
            ...
            .show(ui);
            changed |= StyleSection { config }.show(ui);
            changed |= RowsSection { config }.show(ui);
            changed |= DetailSection { config }.show(ui);
            changed |= NumbersSection { config }.show(ui);
            changed |= ImbalanceSection { config }.show(ui);
            changed |= MarksSection { config }.show(ui);
            let footer = FollowDefaultFooter { customized };
            if footer.show(ui) {
...
/// A section's opening: the rule above it and its heading.
fn section_heading(ui: &mut egui::Ui, heading: &str) {
    ui.separator();
    ui.heading(heading);
}

/// A muted inline caption in front of a value.
...
/// Named setups: load, delete and save the knobs as one stance.
struct PresetsSection<'a> {
    config: &'a mut FootprintConfig,
    presets: &'a mut PresetStore,
    presets_path: &'a Path,
    name_draft: &'a mut String,
}

impl PresetsSection<'_> {
...
/// The style row, and the knobs of the style in force.
struct StyleSection<'a> {
    config: &'a mut FootprintConfig,
}
...
/// How fine the price bands may get.
struct RowsSection<'a> {
    config: &'a mut FootprintConfig,
}

impl RowsSection<'_> {
    fn show(self, ui: &mut egui::Ui) -> bool {
        section_heading(ui, "Rows");
        ui.add(
            egui::Slider::new(&mut self.config.profile_row_px, PROFILE_ROW_PX_RANGE)
...
/// How much candle width each detail level waits for.
struct DetailSection<'a> {
    config: &'a mut FootprintConfig,
}

impl DetailSection<'_> {
    fn show(self, ui: &mut egui::Ui) -> bool {
        section_heading(ui, "Detail");
        ui.add(
            egui::Slider::new(&mut self.config.detail_scale, DETAIL_SCALE_RANGE)
...
/// The digits: per-row quantities and the per-bar delta chips.
struct NumbersSection<'a> {
    config: &'a mut FootprintConfig,
}

impl NumbersSection<'_> {
    fn show(self, ui: &mut egui::Ui) -> bool {
        let config = self.config;
        let mut changed = false;
        section_heading(ui, "Numbers");
        changed |= ui
...
/// What counts as an imbalance: the ratio, the size floor and the stack.
struct ImbalanceSection<'a> {
    config: &'a mut FootprintConfig,
}

impl ImbalanceSection<'_> {
    fn show(self, ui: &mut egui::Ui) -> bool {
        let config = self.config;
        let mut changed = false;
        section_heading(ui, "Imbalance");
        ui.horizontal(|ui| {
            caption(ui, "dominance ≥");
...
/// The marks that survive zoom-out: the POC and the extreme-ratio badge.
struct MarksSection<'a> {
    config: &'a mut FootprintConfig,
}
...
/// The way back to the window default; `true` when the trader took it.
struct FollowDefaultFooter {
    customized: bool,
}

impl FollowDefaultFooter {
    fn show(self, ui: &mut egui::Ui) -> bool {
        ui.separator();
        let reset = ui
            .add_enabled(
                self.customized,
                egui::Button::new("follow the default again"),
            )
            .on_hover_text(
