//! The composition root: what the process does before a window exists.
//!
//! `main` is one line in `main.rs`; everything it has to read, refuse or
//! spawn first lives here, beside the launch inputs it reads.
...
fn init_tracing(format: super::LogFormat) {
    if format == super::LogFormat::Json {
        let filter =
...
fn run_dump_subcommand(argument: &str) -> bool {
    match argument {
        "--dump-capability-inventory" => {
            emit(crate::control::inventory::capability_inventory_markdown());
            true
        }
        "--dump-retry-matrix" => {
            emit(crate::control::retry_matrix::retry_matrix_markdown());
            true
        }
        "--dump-hook-registry" => {
            emit(crate::hooks::hook_registry_markdown());
            true
        }
...
pub(crate) fn run() -> eframe::Result {
    ...
    // feature.
    let startup = super::LaunchConfig::capture(|name| std::env::var_os(name));
    super::install(startup.paths.clone());
    ...
    let control = crate::app::control_host::ControlLaunch::capture(|name| std::env::var_os(name));
    ...
    let environment: Vec<String> = std::env::vars_os()
        ...
        .collect();
    if let Some(reason) = super::persistence_refusal(
        environment.iter().map(String::as_str),
        &crate::hooks::declared_names(),
    ) {
        crate::store_home::refuse_writes(reason);
    }

    // Feed and asset are configuration, not constants. A malformed external
    // config is fatal and surfaced, never silently ignored.
    let (mut config, source) = match crate::config::load(startup.config_path.as_deref()) {
        Ok(loaded) => loaded,
        Err(e) => {
            tracing::error!(
                target: "quantick::app",
    ...
    if let Some(rescue) = crate::store_home::consolidate_once()
        && rescue.copied > 0
    {
        tracing::info!(
            target: "quantick::app",
    ...
    let workspace = crate::ui_state::load(&crate::ui_state::default_path()).restore(&config);
    ...
    let launch = crate::app::AppLaunch {
        #[cfg(feature = "scenario-harness")]
        scenario,
    ...
    eframe::run_native(
        ...
        Box::new(move |cc| {
            ...
            let mut app = crate::app::QuantickApp::new_with_workspace(
