...
//!
//! The paper account journals every close to a folder, and a backtest that
//! drives one must hand it a folder nobody else will read. The same helper
//! `crates/mcp/tests/common/mod.rs` carries, in this crate's own copy: a
//! temporary path keyed on `std::process::id()` alone is one a later run can
//! inherit populated, because the operating system reuses process ids. It
//! lives under `tests/common/` because cargo compiles every file directly in
//! `tests/` as a test binary of its own; a subdirectory module is support.
...
impl ScratchDir {
    /// A fresh directory named `quantick-backtest-<pid>-<nanos>-<counter>-<label>`.
    ///
