...
use crate::scene::{CONTROLS_SCOPE_ID as SCENE_CONTROLS_SCOPE_ID, SceneSnapshot};
use quantick_control_host::authority::EVIDENCE_PERMISSION_ID;
use quantick_control_host::events::read_page;
use quantick_control_host::journal::{EventJournal, EventPage};
use quantick_control_host::projection::{SerializedSnapshotCapture, SnapshotCapture};
use quantick_control_host::system::SystemSnapshot;
use quantick_control_host::wire::{known_error, wire_usize};
use quantick_stores::config::{
    AppConfig, DeclaredLayout, FeedConfig, Mt5SideSource, ProviderKind, split_host_port,
};
...
pub use image::{RawScreenshot, ScreenshotPixels};
...
pub const SCREENSHOT_PERMISSION_ID: &str = "observe.screenshot";

/// The scope one projection publishes the trader's own words under.
pub const USER_TEXT_PERMISSION_ID: &str = "observe.user_text";
...
pub const GRAPHICS_BACKEND: &str = "glow";
...
pub const SCREENSHOT_GAP_SUBJECT: &str = "screenshot";
...
/// silence.
pub const MAX_CONFIGURATION_SYMBOLS: usize = 64;
...
pub const MAX_UNAVAILABLE_FIELDS: usize = 256;
...
#[derive(Clone, Debug, Eq, Ord, PartialEq, PartialOrd, Serialize, Deserialize, JsonSchema)]
pub struct EvidenceUnavailableField {
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize, JsonSchema)]
pub struct EvidenceControlRegion {
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize, JsonSchema)]
pub struct EvidenceFeedConfiguration {
    pub id: String,
    pub name: String,
    pub provider: String,
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize, JsonSchema)]
pub struct EvidenceMetaTraderConfiguration {
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize, JsonSchema)]
pub struct EvidencePaperConfiguration {
...
pub struct EvidenceCapture {
    ...
    pub events: EventPage,
    /// This build's identity, taken by the application: every manifest and
    /// every document carries it whatever scopes were named.
    pub system: SystemSnapshot,
    pub configuration: EvidenceConfiguration,
...
pub const USER_PROSE_PAYLOAD_KEYS: &[&str] = &["note", "message", "text", "label", "name", "title"];
...
pub fn redact_event_prose(mut page: EventPage) -> EventPage {
...
pub fn redact_prose_value(value: &mut Value) {
    match value {
        Value::Object(map) => {
...
pub fn canonical_bytes(document: &EvidenceDocument) -> Result<Vec<u8>, ControlError> {
...
pub fn screenshot_gap(reason: &str) -> EvidenceGap {
    EvidenceGap {
...
pub fn region_gap(reason: &str) -> EvidenceGap {
    EvidenceGap {
...
pub fn fixed_gaps() -> Vec<EvidenceGap> {
    vec![
        EvidenceGap {
...
pub fn user_text_gap(source_scopes: &BTreeSet<PermissionId>) -> EvidenceGap {
...
pub fn collect_unavailable_fields(
...
pub fn redact_configuration(config: &AppConfig) -> (EvidenceConfiguration, Vec<EvidenceGap>) {
...
pub fn redact_feed(feed: &FeedConfig, gaps: &mut Vec<EvidenceGap>) -> EvidenceFeedConfiguration {
...
pub fn split_listen_addr(addr: &str) -> (Option<u16>, bool) {
...
pub const fn provider_id(provider: ProviderKind) -> &'static str {
    match provider {
...
pub const fn side_source_id(source: Mt5SideSource) -> &'static str {
    match source {
...
/// them.
pub fn layout_id(layout: DeclaredLayout) -> Option<String> {
...
pub const ALWAYS_AGGREGATED_PERMISSION_IDS: &[&str] = &[
...
mod tests {
    ...
    fn scope_value(value: Value) -> quantick_control_host::projection::SerializedScope {
        quantick_control_host::projection::SerializedScope {
