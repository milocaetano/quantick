...
pub const CONSOLIDATED_MARKER: &str = ".consolidated";
...
pub const MAX_IMPORT_RENAMES: usize = 99;
...
/// cannot find is not.
pub fn default_trades_dir(documents: Option<PathBuf>) -> PathBuf {
...
pub fn resolve_startup_home(
    ...
    if summary.failed == 0 && summary.source_unreadable == 0 {
        ...
        if stored.is_some() {
            crate::state::clear_trades_dir(state_path);
        }
...
pub fn write_consolidated_marker(marker: &Path) {
    let written = marker
        .parent()
...
pub fn import_toast(summary: &ImportSummary) -> String {
...
pub fn consolidate_into(dest: &Path, sources: &[PathBuf]) -> ImportSummary {
...
pub fn is_history_file(path: &Path) -> bool {
    path.extension()
...
/// after.
pub fn copy_symbol_folder(folder: &Path, dest: &Path, summary: &mut ImportSummary) {
...
/// or it stays behind.
pub fn copy_loose_file(path: &Path, dest: &Path, summary: &mut ImportSummary) {
    ...
    match symbol {
        Some(symbol) => {
            let folder = dest.join(crate::format::sanitize_symbol(&symbol));
...
pub fn copy_one(file: &Path, folder: &Path, summary: &mut ImportSummary) {
...
pub fn try_copy_one(file: &Path, folder: &Path) -> std::io::Result<Outcome> {
...
pub fn import_name(name: &OsStr, suffix: usize) -> PathBuf {
...
mod tests {
    ...
    fn the_rescue_runs_once_and_then_the_pick_wins_again() {
        ...
        assert_eq!(
            crate::state::load(&state_path).trades_dir,
            None,
