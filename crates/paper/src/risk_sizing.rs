...
/// applies.
pub type Capital = BTreeMap<String, Decimal>;
...
pub type InstrumentBook = BTreeMap<String, InstrumentMoney>;
...
pub struct InstrumentMoneyRecord {
...
impl InstrumentMoneyRecord {
    ...
    pub fn to_money(&self, symbol: &str) -> Option<InstrumentMoney> {
...
pub fn book_from_records(records: &BTreeMap<String, InstrumentMoneyRecord>) -> InstrumentBook {
    records
        .iter()
...
pub fn records_from_book(book: &InstrumentBook) -> BTreeMap<String, InstrumentMoneyRecord> {
    book.iter()
        .map(|(symbol, money)| {
            (
                symbol.clone(),
...
pub fn records_from_capital(capital: &Capital) -> BTreeMap<String, String> {
    capital
        .iter()
...
pub fn capital_from_records(records: &BTreeMap<String, String>) -> Capital {
    records
        .iter()
...
impl RiskState {
    ...
    pub fn derived_quantity(&self) -> Option<Decimal> {
        match self {
    ...
    pub fn blocks_entry(&self, lock: bool) -> bool {
...
pub fn with_restated_risk(state: RiskState, restated: Money) -> RiskState {
    match state {
        RiskState::Sized {
            quantity,
            outcome,
...
pub fn parse_hook(spec: &str) -> Option<RiskHook> {
