...
/// [`the cockpit home::COCKPIT_STORES`].
...
/// choice.
#[must_use]
pub fn load(path: &Path) -> PaperState {
...
pub fn clear_trades_dir(path: &Path) {
...
mod tests {
    ...
    /// A store of its own per test, inside the thread's own directory so
    /// the file goes with the thread.
