...
async fn binance_automatic_reconnect_reports_loss_on_the_host_channel() {
    ...
    assert!(matches!(&received[1], FeedEvent::Live(t) if t.agg_id == 10));
    assert!(matches!(
        &received[2],
    ...
    assert!(matches!(&received[3], FeedEvent::Live(t) if t.agg_id == 14));
    assert!(matches!(&received[4], FeedEvent::Live(t) if t.agg_id == 15));
    assert!(
        rx.try_recv().is_err(),
