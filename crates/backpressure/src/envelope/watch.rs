//! The live envelope, observed: worker queues and retained tape set against
//! [`super`]'s figures, and the warnings a reading past them earns.
//!
//! The window takes the reading at its health-summary cadence — every pane
//! of every tab, because an overflow in a background tab's pane is an
//! overflow all the same — and hands it here; nothing in this module knows
//! what a pane is. See [`rate_class`] for the one figure that is the active
//! tab's alone.
...
impl EnvelopeReading {
    ...
    pub fn add(&mut self, progress: &ProgressSnapshot) {
    ...
    pub fn retained_excess(&self) -> Option<usize> {
...
/// `None` inside it.
pub fn outside_envelope(retained: usize) -> Option<usize> {
    retained
...
pub fn rate_class(window_trades_per_s: f64, active_depth_per_s: f64) -> &'static str {
...
impl EnvelopeWatch {
    ...
    /// repeat forever.
    pub fn warn(&mut self, reading: &EnvelopeReading) {
...
mod tests {
    use super::*;
    use crate::progress::{Counts, ProgressClock, WorkerProgress};

    /// A clock that never reads the time: the figures here are counts.
    struct Still;
    impl ProgressClock for Still {
    ...
    fn snapshot(instance: u64, counts: Counts) -> ProgressSnapshot {
        let mut progress = WorkerProgress::with_clock(std::sync::Arc::new(Still))
            .observer()
            .snapshot();
