...
use crate::backlog::never;
use crate::backlog::{Admitted, Merge, Parked};
...
pub trait ProgressClock: Send + Sync {
...
#[derive(Clone)]
pub struct ProgressObserver(Rc<OwnerProgress>);
...
impl WorkerProgress {
    pub fn with_clock(clock: Arc<dyn ProgressClock>) -> Self {
    ...
    pub fn observer(&self) -> &ProgressObserver {
        &self.observer
    }
    pub fn consumer(&self) -> Arc<SharedProgress> {
    ...
    /// Bind without folding: what a test that wants every command delivered
    /// as sent asks for.
    #[cfg(any(test, feature = "test-support"))]
    pub fn bind<T>(self, sender: SyncSender<T>) -> ObservedSender<T> {
    ...
    pub fn bind_merging<T>(self, sender: SyncSender<T>, merge: Merge<T>) -> ObservedSender<T> {
        ObservedSender {
            sender,
...
impl<T> ObservedSender<T> {
    ...
    pub fn send(&self, command: T) -> Result<(), SendError<T>> {
...
impl SharedProgress {
    ...
    pub fn finish(&self, mailbox: bool) {
        let now = self.clock.now_ns();
        {
            let mut state = self.lock();
            let Ledger { counts, valid, .. } = &mut *state;
    ...
    /// The worker's lifetime: dropped when its thread ends, and told by
    /// `unwinding` — `std::thread::panicking`, from the caller that owns the
    /// thread — whether it ended by unwinding.
    pub fn lifecycle(self: &Arc<Self>, unwinding: fn() -> bool) -> Lifecycle {
        Lifecycle(Arc::clone(self), unwinding)
    }
}

pub struct Lifecycle(Arc<SharedProgress>, fn() -> bool);
...
impl<'a> Coalescing<'a> {
    pub fn new(progress: &'a SharedProgress) -> Self {
        Self {
            progress,
            armed: true,
            inputs: 0,
