...
//!
//! Indexes in this repository produced from the code rather than written
//! beside it: `docs/control-plane/capability-inventory.md`, emitted by
//! `quantick-app --dump-capability-inventory`, the UI behaviour and retry
//! matrices beside it, and the hook registry under
...
/// The committed generated UI behaviour matrix.
pub const MATRIX_PATH: &str = "docs/control-plane/ui-behaviour-matrix.md";

/// The marker the generated behaviour matrix carries.
const MATRIX_MARKER: &str =
    "<!-- generated by `quantick-app --dump-ui-behaviour-matrix`; do not edit -->";

const REMEDY_MATRIX_REGENERATE: &str = "Regenerate the matrix instead of editing it: \
     `cargo run -p quantick-app -- --dump-ui-behaviour-matrix > \
     docs/control-plane/ui-behaviour-matrix.md`, then commit the result.";

/// The committed generated retry matrix.
pub const RETRY_MATRIX_PATH: &str = "docs/control-plane/retry-matrix.md";

/// The marker the generated retry matrix carries.
const RETRY_MATRIX_MARKER: &str =
    "<!-- generated by `quantick-app --dump-retry-matrix`; do not edit -->";

const REMEDY_RETRY_MATRIX_REGENERATE: &str = "Regenerate the retry matrix instead of editing \
     it: `cargo run -p quantick-app -- --dump-retry-matrix > \
     docs/control-plane/retry-matrix.md`, then commit the result.";
...
pub fn check(root: &Path) -> Vec<Finding> {
    ...
    check_retry_matrix(root, &mut findings);
...
/// capability is added: the inventory lists a capability that can change
/// state, and the retry matrix has no row saying how a client reconciles it.
/// The other direction is checked too — a row for a capability the inventory
/// does not list as mutable, and a row whose readback names a capability the
/// inventory does not register.
fn check_retry_matrix(root: &Path, findings: &mut Vec<Finding>) {
    let Ok(matrix) = std::fs::read_to_string(root.join(RETRY_MATRIX_PATH)) else {
        findings.push(Finding::new(
            format!("{RETRY_MATRIX_PATH}: missing — the generated retry matrix is not there"),
            REMEDY_RETRY_MATRIX_REGENERATE,
        ));
        return;
    };
    if !matrix.contains(RETRY_MATRIX_MARKER) {
        findings.push(Finding::new(
            format!("{RETRY_MATRIX_PATH}: the generated marker line is gone"),
            REMEDY_RETRY_MATRIX_REGENERATE,
        ));
        return;
    }
    let Ok(inventory) = std::fs::read_to_string(root.join(INVENTORY_PATH)) else {
        // `check_inventory` above has already reported this.
        return;
    };
    let mut registered = BTreeSet::new();
    let mut mutable = BTreeSet::new();
    for columns in table_rows(&inventory, 5) {
        let id = columns[0].trim().trim_matches('`').to_owned();
        if columns[3].trim() == "no" {
            mutable.insert(id.clone());
        }
    ...
    for columns in table_rows(&matrix, 8) {
        ...
        if !mutable.contains(&id) {
            findings.push(Finding::new(
                format!(
                    "{RETRY_MATRIX_PATH}: a row covers `{id}`, which {INVENTORY_PATH} does not \
                     list as a capability that changes state"
                ),
                REMEDY_RETRY_MATRIX_REGENERATE,
            ));
        }
    ...
    for id in mutable.difference(&covered) {
        findings.push(Finding::new(
            format!(
...
/// The rows of a generated table with exactly `width` columns whose first
/// cell is a backticked identifier — the shape both generators write their
/// capability rows in, and a shape no heading or summary row has.
fn table_rows(document: &str, width: usize) -> impl Iterator<Item = Vec<&str>> {
    document.lines().filter_map(move |line| {
        let columns: Vec<&str> = line.trim_matches('|').split('|').collect();
        (line.starts_with("| `") && columns.len() == width).then_some(columns)
    })
}

/// The cheap half of the UI behaviour matrix's guard.
///
/// `crates/app/src/operability/` holds the authoritative one: it walks the
/// interface's own registries and compares them against the table row by row.
/// This cannot — those registries are types in the slowest crate in the
/// repository, and this crate has no dependencies. What it can do in a second
/// is the comparison that goes stale most often, because it is the one whose
/// author is not looking at the interface at all: a matrix row naming a
/// capability the inventory does not carry.
///
/// That is the drift a *capability* change causes — a rename, a withdrawal, a
/// version bump that moves an identifier — and its author is editing
/// `crates/app/src/control/`, with no reason to think about a behaviour matrix.
/// The interface side of the drift stays with the authoritative test, whose
/// author is editing the interface and paying for that build anyway.
fn check_matrix(root: &Path, findings: &mut Vec<Finding>) {
    let path = root.join(MATRIX_PATH);
    let Ok(matrix) = std::fs::read_to_string(&path) else {
        findings.push(Finding::new(
            format!("{MATRIX_PATH}: missing — the generated UI behaviour matrix is not there"),
            REMEDY_MATRIX_REGENERATE,
        ));
        return;
    };
    if !matrix.contains(MATRIX_MARKER) {
        findings.push(Finding::new(
            format!("{MATRIX_PATH}: the generated marker line is gone"),
            REMEDY_MATRIX_REGENERATE,
        ));
        return;
    }
    ...
    let registered: BTreeSet<&str> = inventory
        .lines()
        .filter_map(|line| line.strip_prefix("| `"))
        .filter_map(|line| line.split_once('`'))
        .map(|(id, _)| id)
        .collect();
    for named in matrix_capabilities(&matrix) {
        if !registered.contains(named.as_str()) {
            findings.push(Finding::new(
                format!(
                    "{MATRIX_PATH}: a row maps a behaviour to `{named}`, which \
                     {INVENTORY_PATH} does not register"
                ),
...
/// The capability identifiers the matrix maps behaviours to.
///
/// The fourth column of a behaviour row, which is an em dash where the row
/// records an exclusion instead. Read positionally rather than by heading
/// because the document is generated: its column order is written once in the
/// renderer, and a renderer that reordered them would fail its own
/// byte-for-byte comparison first.
fn matrix_capabilities(matrix: &str) -> BTreeSet<String> {
...
mod tests {
    ...
    /// The behaviour matrix names only capabilities the inventory carries.
    ///
    /// The same `check` call as the test above, said separately because the
    /// two failures send the reader to different files.
    #[test]
    fn the_behaviour_matrix_names_only_registered_capabilities() {
        let findings: Vec<String> = check(&workspace_root())
            .iter()
            .filter(|finding| finding.line.starts_with(MATRIX_PATH))
            .map(|finding| finding.line.clone())
            .collect();
        assert!(
            findings.is_empty(),
            "the UI behaviour matrix names a capability the inventory does not:\n{}",
            findings.join("\n")
        );
    }

    /// The capability column is read, and the exclusion rows are not mistaken
    /// for it. Without this the em dash of an excluded row would be reported
    /// as a capability nothing registers, which is every excluded row at once.
    #[test]
    fn only_the_capability_column_of_a_behaviour_row_is_read() {
        let matrix = "| Behaviour | What it does | Reach | Capability | Exclusion |\n\
                      | --- | --- | --- | --- | --- |\n\
                      | `layout.tab.create` | Add a tab | strip | `layout.tab.create` | — |\n\
                      | `app.exit` | Close it | File menu | — | `authority` — a ceiling |\n\
                      | `layout.context.collapse` | Fold | Ctrl+0 | \
                      `layout.pane.collapse`, `layout.pane.expand` | — |\n";
        let named = matrix_capabilities(matrix);
        assert_eq!(
            named.into_iter().collect::<Vec<_>>(),
            vec![
                "layout.pane.collapse".to_owned(),
                "layout.pane.expand".to_owned(),
                "layout.tab.create".to_owned(),
            ]
        );
    }
