...
//! is; `crates/sources/src/history_reach.rs` walks back from the chart's oldest bar
...
fn rust_constant(source: &str, name: &str) -> i64 {
        ...
        .unwrap_or_else(|| {
            panic!(
                "`{name}` is not declared as a `pub const … : i64` in \
                 crates/sources/src/history_reach.rs. If it was renamed or retyped, say so here \
    ...
    let expression = line
        ...
        .expect("a split always yields a first part")
        .trim();
    product_of_literals(expression, name, "crates/sources/src/history_reach.rs")
}
...
fn the_bridge_and_the_app_measure_a_session_the_same_way() {
    ...
    let rust_source = std::fs::read_to_string(root.join("crates/sources/src/history_reach.rs"))
